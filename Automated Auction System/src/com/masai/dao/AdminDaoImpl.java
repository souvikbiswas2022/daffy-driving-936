...
public class AdminDaoImpl implements AdminDao {
	...
	public String resisterAdmin(Admin a)  throws AdminException {
		...
		try (Connection connection = DBUtil.provideConnection()) {
			...
			throw new AdminException("Sorry duplicate email '"+a.getEmail()+"'");
		}
	...
	public ArrayList<Item> getAllItems() throws ItemException {
		...
		try (Connection connection = DBUtil.provideConnection()) {
			...
			while (rs.next()) {
				...
				allItems.add(new Item(salerid, itemId, itemName, category, basePrice, quantity, itemStatus, soldDate,
...
//			System.out.println(map);
	...
	public ArrayList<DisputedItem> getDisputeReport() {
		...
		ArrayList<DisputedItem> DisputedItems = new ArrayList<>();

		try (Connection connection = DBUtil.provideConnection()) {

			PreparedStatement ps = connection.prepareStatement("select * from dispute");
			ResultSet rs = ps.executeQuery();

			while (rs.next()) {

				int itemId = rs.getInt("itemid");
				String disReason = rs.getString("disReason");
				String disStatus = rs.getString("disstatus");

				DisputedItems.add(new DisputedItem(itemId, disReason, disStatus));
	...
	public String solveDispute(int itemid) {
		...
		String message = "Dispute not resolve ...";

		try (Connection connection = DBUtil.provideConnection()) {

			PreparedStatement ps = connection.prepareStatement("update dispute set disstatus='RESOLVED' where itemid=?");
	...
	public ArrayList<SoldItem> getDailySellingReport() throws SoldItemException{
		...
		try (Connection connection = DBUtil.provideConnection()) {
			...
			PreparedStatement ps = connection.prepareStatement(	"select i.itemname,b.bname,s.sname from items i inner join buyers b inner join sellers s on b.bid=i.bid and s.sid=i.sid;");
			...
			while (rs.next()) {
				...
				String itemName = rs.getString("itemname");
				String buyerName = rs.getString("bname");
				String sellerName = rs.getString("sname");

				solditems.add(new SoldItem(itemName, buyerName, sellerName));

			}
			

		} catch (SQLException e) {
			throw  new SoldItemException(e.getMessage());
		}

		if(solditems.size() == 0) {
			throw  new SoldItemException("No sold item found ...");
		}
